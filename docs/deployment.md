...
### 5. Move the deploy dir to `/data/my-whole-server`

doco-cd uses `/data` as its data root and the bind mount in `compose.yaml` maps it 1:1 to the host so relative paths resolve correctly.

```sh
sudo mv ~/server /data/my-whole-server
cd /data/my-whole-server
git fetch && git checkout master && git reset --hard origin/master
```

### 6. Final manual deploy
...
### 9. Clean up old GitHub Actions secrets
...
| Manual rollback | SSH to host, `git -C /data/my-whole-server checkout <good-sha>`, `SECRETS_DIR=/etc/my-whole-server/ docker compose up -d --build` |
